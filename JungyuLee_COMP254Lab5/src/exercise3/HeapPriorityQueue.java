...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
    ...
    */
    ...
    public static void main(String[] args)
    {
        ...
        // Remove elements one by one and print the state of the queue after each removal
        while (!pq.isEmpty()) {
            Entry<Integer, String> min = pq.removeMin();
            System.out.println("Remove: (" + min.getKey() + ", " + min.getValue() + ")");
     ...
     * Hint: Do a single upward swap and recur (if necessary).
     */
    protected void upheap(int j) {
        if (j > 0) { // base case: j == 0 means that we've reached the root since j is the index of the current node
            int p = parent(j);
            if (compare(heap.get(j), heap.get(p)) < 0) { // heap property violated
                swap(j, p); // swap the child with its parent
                upheap(p); // recur on the parent
            }
        }
    }
