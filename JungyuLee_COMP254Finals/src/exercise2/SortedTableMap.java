...
public class SortedTableMap<K,V> extends AbstractSortedMap<K,V> {
    ...
    /**
     * Exercise 2
     * The use of null values in a map is problematic,
     * as there is then no way to differentiate
     * whether a null value returned by the call get(k) represents the legitimate value of an entry (k, null),
     * or designates that key k was not found.
     * The java.util.Map interface includes a boolean method, containsKey(k), that resolves any such ambiguity.
    ...
    public boolean containsKey(K key) {
        ...
        // if (high < low) return high + 1; // no entry qualifies -> IndexOutOfBoundsException
        if (index == size() || compare(key, table.get(index)) != 0) // compare method returns 0, that means the two keys are equal
     ...
     * @return entry with grecatest key (or null if map is empty)
     */
    @Override
    ...
    public static void main(String[] args) {
        SortedTableMap<String, Integer> sortedTableMap = new SortedTableMap<>();

        sortedTableMap.put("A", null);

        // get Test
        System.out.println("-----get Test-----");
        System.out.println("key O value null =\t" + sortedTableMap.get("A"));
        System.out.println("key X value X =\t" + sortedTableMap.get("E"));

        System.out.println();
        // containsKey Test
        System.out.println("-----containsKey Test-----");
        System.out.println("key O value null =\t" + sortedTableMap.containsKey("A"));
        System.out.println("key X value X =\t" + sortedTableMap.containsKey("E"));
    }
